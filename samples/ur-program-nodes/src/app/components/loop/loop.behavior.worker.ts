...
const behaviors: ProgramBehaviors<SampleLoopNode> = {
    ...
    programNodeLabel: (node: SampleLoopNode): AdvancedProgramLabel => {
        switch (node.parameters.type) {
            case 'always':
                return [{ type: 'secondary', value: `Always` }];
            case 'xtimes':
                return [{ type: 'secondary', value: `${node.parameters.xtimes} Times` }];
            case 'expression':
                return [{ type: 'secondary', value: `While: ${node.parameters.expression}` }];
            default:
                return [{ type: 'secondary', value: '' }];
        }
    },
